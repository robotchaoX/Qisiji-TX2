...
extern int
    range_high; //作为高度是否正确的判断条件,同时作为车筐边沿提取的上下边界值
...
void handleFrame(TY_FRAME_DATA *frame, void *userdata, MainWindow &w,
    ...
    if (!color.empty()) {
        ...
        // TYUndistortImage accept TY_IMAGE_DATA from TY_FRAME_DATA , pixel
        // format RGB888 or MONO8 you can also use opencv API cv::undistort to
        // do this job.
        ASSERT_OK(TYUndistortImage(&pData->color_intri, &pData->color_dist,
                                   NULL, &src, &dst));
    ...
    if (!p3d.empty() && !color.empty()) {
        ...
        cv::Mat temp;
        // you may want to use median filter to fill holes in projected depth
        // image or do something else here
    ...
    if (w.detec) {
        ...
        box_grub(
            cloud_3f, box_world, box_UPworld, linePoints,
        ...
        if (linePoints.size() > 50) {
            ...
            // fallPointWorldFind(box_world,
            // downfallPoints_2D,fallPoints_world);
            /*thread t1(fallPointWorldFind, ref(box_world),
            ref(upfallPoints_2D), ref(fallPoints_world)); thread
            t2(fallPointWorldFind, ref(box_world), ref(downfallPoints_2D),
            ref(fallPoints_world)); t1.join(); t2.join();*/
            //喷撒规划
            ...
            thread t4(co_transfer_2, ref(fallPoints_world),
                      ref(fallPoints_pixel));
            t3.join();
            t4.join();
            //绘制边框,落点
            ...
            contr1.traversal_control(fallPoints_world, height_of_basket,
                                     range_up, w, fall_step); // heng向控制
            // contr1.vertical_control(fallPoints_2D);//zong向控制
            contr1.vertical_control_Vision(fallPoints_2D, box_UPworld,
                                           x_axisFallPoint);
...
int main(int argc, char *argv[]) {
    ...
    LOGD("give it back to user, user should call TYEnqueueBuffer to re-enqueue "
         "it.");
    DepthRender render;
    ...
    LOGD("=== Read color rectify matrix"); ///读取彩色矫正矩阵
    {
        ...
        ret |=
            TYGetStruct(hDevice, TY_COMPONENT_RGB_CAM, TY_STRUCT_CAM_INTRINSIC,
    ...
    while (!exit_main) {
        ...
        if (err != TY_STATUS_OK) {
            ...
            cout << "get_frame_time="
                 << (get_frame_end - get_frame_start) / 1000 << "ms" << endl;
        }
        clock_t while_end = clock();
        cout << "while_time=" << (while_end - while_start) / 1000 << "ms"
             << endl;
    }
    key_ = false;
    can_closed();
