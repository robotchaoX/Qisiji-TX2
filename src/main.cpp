...
void handleFrame(TY_FRAME_DATA *frame, void *userdata, MainWindow &w,
  ...
  depth_1 = depth_;
  depth_1.convertTo(depth_1, CV_8UC1,
                    32.0 /
                        255); //进行位深度的转换,2->1字节
  ...
  if (w.detec) {
    ...
    if (linePoints.size() > 50) {
      ...
      */
      ...
      contr1.traversal_control(fallPoints_world, height_of_basket, symbol_step,
                               w); //纵向控制
...
int main(int argc, char *argv[]) {
  ...
  LOGD("      To avoid copying data, we pop the framebuffer from buffer queue "
       "and");
  LOGD("      give it back to user, user should call TYEnqueueBuffer to "
       "re-enqueue it.");
  DepthRender render;
