...
void control::traversal_control(vector<Point3f> &fallpoint_world,
    ...
    if (fallpoint_world.size() == 0) {
        ...
        for (int i = 0; i < fallpoint_world.size(); ++i) {
            ...
            if ((fallpoint_world[i].z <= height_of_basket + symbol_step / 2) &&
                (i != 0)) {
                cout << "i=" << i << endl;
                if ((fallpoint_world[i].x <= (fall_step / 2)) &&
                    (fallpoint_world[i].x >= -(fall_step / 2))) {
                    ...
                    break;
                } else if ((fallpoint_world[i].x) > (fall_step / 2) &&
                           (++count_r > 2)) {
                    count_l = 0;
                    ...
                    /*                else
                       if(fallpoint_world[i].x<5*(fall_step/2)){
...
void control::vertical_control(
    const vector<Point2f> &fallPoints_2D) //喷头角度与落点的关系
{
    ...
    if (SensorAngle > (angle_reslut - 100) &&
        SensorAngle < (angle_reslut + 100)) {
...
void control::vertical_control_Vision(const vector<Point2f> &fallPoints_2D,
    ...
    if (box_UPworld.size()) {
        ...
        if (ActualFallPointDastance > (IdealFallPointDastance - 100) &&
            ActualFallPointDastance < (IdealFallPointDastance + 100)) {
            CAN_data[2] = 0x00; //??stop f1 07 00 00 00 00 00 00
            // zan ting daoliaoban
